...
export class QuestionGeneratorService {
	...
	async generateDailyQuestion(
		...
		try {
			...
			logger.info(
				`Generated daily question - Type: ${parsedQuestion.type}, Difficulty: ${parsedQuestion.difficulty}`
			);
...
- "💡 What's the difference between..."`;
	...
	private getFallbackQuestion(difficulty: string): GeneratedQuestion {
		const fallbackQuestions = {
			...
			intermediate:
				"🤔 Describe your perfect day using at least 5 different adjectives. Let's see your creativity!",
			advanced:
				"💭 If you could have dinner with any English-speaking historical figure, who would it be and what would you discuss?",
		};

		return {
			question:
				fallbackQuestions[difficulty as keyof typeof fallbackQuestions] ||
				fallbackQuestions.intermediate,
			type: "conversation",
			difficulty,
	...
	async generateThemedQuestion(
		...
		const themeDescription =
			themes[theme as keyof typeof themes] || "general English learning";
